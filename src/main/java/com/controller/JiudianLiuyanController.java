...
//            jiudianLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<JiudianLiuyanEntity> queryWrapper = new EntityWrapper<JiudianLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        JiudianLiuyanEntity jiudianLiuyanEntity = jiudianLiuyanService.selectOne(queryWrapper);
        jiudianLiuyan.setUpdateTime(new Date());
        if(jiudianLiuyanEntity==null){
...
//                            jiudianLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
...
//                            jiudianLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
